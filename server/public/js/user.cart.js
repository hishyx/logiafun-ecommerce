...
async function removeFromCart(e, itemId) {
  ...
  if (response.ok) {
    ...
    if (result.calculations) {
      ...
      if (gstEl) gstEl.innerText = `₹${result.calculations.gst || 0}`;
...
// Redundant Add-to-Cart logic removed. Handled by product.details.js engine.
      ...
      if (response.ok) {
        ...
        if (shippingEl) {
          shippingEl.innerText =
            result.calculations.shipping && result.calculations.shipping > 0
              ? `₹${result.calculations.shipping}`
              : "Free";
        }
